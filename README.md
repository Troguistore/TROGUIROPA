...
.shipping-section{background:linear-gradient(180deg,#fff 0%,#fff8f3 100%);padding:38px 16px;border-top:1px solid var(--border);border-bottom:1px solid var(--border)}
.shipping-inner{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:1.1fr .9fr;gap:34px;align-items:center}
.shipping-text h2{font-family:'Poppins',sans-serif;font-weight:900;font-size:clamp(20px,2.6vw,28px);color:var(--dark);margin-bottom:10px;line-height:1.25}
.shipping-text h2 span{color:var(--orange)}
.shipping-text p{color:var(--gray);font-size:14.5px;line-height:1.6;margin-bottom:18px}
.carrier-badges{display:flex;flex-wrap:wrap;gap:10px}
.carrier-chip{display:flex;align-items:center;gap:8px;background:#fff;border:2px solid var(--border);border-radius:14px;padding:10px 16px;font-weight:800;font-size:13.5px;color:var(--dark);box-shadow:0 3px 10px rgba(0,0,0,.05);transition:.25s;animation:chipFloat 3.2s ease-in-out infinite}
...
.carrier-chip .cc-dot{width:9px;height:9px;border-radius:50%;background:var(--green);animation:blink 1.3s infinite}
.shipping-trust{margin-top:16px;display:inline-flex;align-items:center;gap:8px;background:#f0fff4;border:1px solid var(--green);color:#006630;font-weight:800;font-size:13px;padding:9px 15px;border-radius:12px}
.shipping-img-wrap{position:relative;border-radius:22px;overflow:hidden;box-shadow:var(--shadow-hover)}
.shipping-img-wrap img{width:100%;display:block;transition:.5s}
.shipping-img-wrap:hover img{transform:scale(1.04)}
@media(max-width:800px){.shipping-inner{grid-template-columns:1fr}.shipping-img-wrap{order:-1}}
...
function openModal(id){
  ...
  document.getElementById('modal-content').innerHTML=`
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
      ...
      <button onclick="shareProduct('${p.id}')" style="background:#f2f2f2;border:none;color:var(--dark);font-weight:800;font-size:12px;padding:7px 13px;border-radius:20px;cursor:pointer;display:flex;align-items:center;gap:5px">🔗 Compartir producto</button>
    </div>
